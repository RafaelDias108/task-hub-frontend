<template>
    <v-dialog v-model="showModal" width="auto">
        <v-card min-width="500" title="Adicionar uma nova tarefa">
            ...
            <template v-slot:text>
                <v-form>
                    <v-row class="mx-2 my-2">
                        <v-col cols="12">
                            <v-text-field label="Tarefa" required clearable v-model="formTask.title_task"
                                :disabled="props.isLoading"></v-text-field>
                            <v-text-field label="Descrição da tarefa" required clearable
                                v-model="formTask.description_task" :disabled="props.isLoading"></v-text-field>
                            <v-text-field type="date" label="Data da tarefa" required clearable
                                v-model="formTask.date_task" :disabled="props.isLoading"></v-text-field>
                            <v-text-field type="time" label="Hora da tarefa" required clearable
                                v-model="formTask.time_task" :disabled="props.isLoading"></v-text-field>
